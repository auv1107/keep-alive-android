...
public class WhiteListIntentFactory {
...
//            if (!DaemonEnv.sInitialized) return new ArrayList<>();
            ...
            xiaomiGodIntent.putExtra("package_name", sApp.getPackageName());
            ...
            //三星 5.0/5.1 自启动应用程序管理
            Intent samsungLIntent = sApp.getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
...
//            if (!DaemonEnv.sInitialized) return "";
            ...
            ApplicationInfo ai;
            try {
                pm = sApp.getPackageManager();
                ai = pm.getApplicationInfo(sApp.getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName = sApp.getPackageName();
            }
        }
